...
<script type="text/html" id="action">
    ...
    <a class="layui-btn layui-btn-normal layui-btn-mini" lay-event="action">生成操作</a>
    ...
    layui.use('table', function() {
        ...
        table.on('tool(list)', function(obj){
            ...
            if(obj.event === 'authopen'){
                ...
                layer.confirm('您确定要删除该记录吗？', function(index){
                    ...
                    $.post("<?php echo url('ruleDel'); ?>",{id:data.id},function(res){
                        ...
                        if(res.code==1){
                            ...
                            location.href="<?php echo url('adminRule'); ?>";
                        }else{
                ...
                layer.confirm('您确定要生成操作权限吗？', function(index){
                    var loading = layer.load(1, {shade: [0.1, '#fff']});
                    $.post("<?php echo url('ruleAct'); ?>",{id:data.id},function(res){
                        layer.close(loading);
                        if(res.code==1){
                            layer.msg(res.msg,{time:1000,icon:1});
                        }else{
